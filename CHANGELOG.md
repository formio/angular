...
- cd4f530: FIO-11509 added zoneless support for angular 21
- 8353164: Ensure that the change detection is properly triggered for Angular 21 applications.
- 9fe8ca5: Make `@formio/angular` change-detection-mode agnostic (works identically under zoned and zoneless CD) and always wire the global SDK from the app's config — with no app-level change required.
  - Add `ChangeDetectorRef.markForCheck()` to the alert-mutation paths that previously relied only on `NgZone.run` (`onSubmit` success alert, `onPrevPage`/`onNextPage` alert clear, report `fetchDataError`). These rendered stale under Angular's zoneless change detection; `markForCheck()` is a no-op-equivalent under zoned CD, so existing zoned apps are unaffected.
  - The `FormioModule` constructor now reads the injected `FormioAppConfig` and calls `Formio.setBaseUrl`/`setProjectUrl` at bootstrap. Because every app already `imports: [FormioModule]`, the SDK is now configured regardless of HOW the config was provided — including the common `{ provide: FormioAppConfig, useValue: {...} }` form, which skips the `FormioAppConfig` constructor where these calls also live. This fixes the "SDK never configured" gap without requiring any app change (no `forRoot`, no `FORMIO_CONFIG` rewrite). The dependency is `@Optional()`, so apps that provide no config are unaffected.
  - Correct the advertised `peerDependencies` floor to Angular `^17.0.0` (built-in control-flow `@if`/`@for` in templates already required v17+; the previous `^16.0.0` floor was never consumable).
...
- 5a5cb03: FIO-11509 added zoneless support for angular 21 and fix elongated spinner

## 11.0.1

### Patch Changes

- 7382c03: Dependency bumps xmldom, lodash, dompurify
- 8d19244: fix formview submissions edit load
...
- 5790024: FIO-10726: upgraded Angular version to 20 for @formio/angular, formmanager and form view pro

### Patch Changes

- 5e79dfa: FIO-11054: upgraded to Angular 21
- 8a5e029: Fixing issue with @formio/angular to support ngx-bootstrap@21 along with Angular 21
- e785ece: bump lodash to 4.17.23
- 4357b08: FIO-11243 FVP and FMG updates
- 8eef44b: Add support to standalone components
...
- e59e35c: formio/angular added missing type def of string to language
